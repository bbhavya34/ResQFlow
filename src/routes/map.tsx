...
export const Route = createFileRoute("/map")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Layered India flood-response map with SOS, resources and safe routing." },
    ],
  }),
  component: MapPage,
});
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="panel p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            NetworkX shortest path over an OpenStreetMap road/waterway graph with flooded edges penalised.
          </p>
          <dl className="mt-3 space-y-1.5 text-xs">
            <Row k="Route" v={`${SAFE_ROUTE.resourceId} → SOS ${SAFE_ROUTE.sosId}`} />
            <Row k="Graph nodes evaluated" v={SAFE_ROUTE.nodes.toLocaleString("en-IN")} />
        ...
        <div className="panel p-4">
          ...
          <ul className="mt-3 space-y-2 text-xs">
            {BLOCKED_ROADS.map((r) => (
              <li key={r.name} className="border-b border-border pb-2 last:border-0">
        ...
        <div className="panel p-4">
          ...
          <ul className="mt-3 space-y-2 text-xs">
            {FLOOD_ZONES.map((z) => (
              <li key={z.id} className="flex justify-between gap-2 border-b border-border pb-2 last:border-0">
                <span>
