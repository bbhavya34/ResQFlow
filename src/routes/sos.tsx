...
export const Route = createFileRoute("/sos")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "SOS Intake & Priority Engine — Aegis Bharat" },
      { property: "og:description", content: "Explainable priority scoring for every Indian flood distress request." },
    ],
  }),
  component: SosPage,
});
...
function SosPage() {
  ...
  const parse = () => {
    const m = raw.trim().match(/^SOS\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s+(\d+)$/i);
    if (!m) {
    ...
    const s: SOS = {
      ...
      status: "NEW",
      factors: [
        { label: "Flood depth", value: 13, max: 25, note: "1.3 m estimated from nearest gauge interpolation" },
        { label: "Vulnerable people", value: 9, max: 20, note: "Children/elderly inferred from household profile" },
        { label: "Population at risk", value: Math.min(15, people), max: 15, note: `${people} persons reported over SMS` },
        { label: "Road inaccessibility", value: 9, max: 15, note: "Approach lane flagged waterlogged by field unit" },
        { label: "Historical damage index", value: 6, max: 10, note: "Ward inundated in previous monsoon" },
        { label: "Livestock exposure", value: 0, max: 5, note: "No livestock reported in the SMS payload" },
        { label: "Resource distance", value: 5, max: 10, note: "Nearest available craft under 6 km" },
      ],
    };
    addSOS(s);
    setSelectedId(id);
    ...
    <div className="space-y-6">
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <div className="panel p-4">
          <SectionTitle title="SMS / low-bandwidth intake" desc="Works on 2G and feature phones — no app required." />
          ...
          {err ? (
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Gateway format: <span className="font-mono">SOS &lt;lat&gt; &lt;lng&gt; &lt;people&gt;</span> — e.g.
              <span className="font-mono"> SOS 9.9312 76.2673 6</span>
            </p>
          )}
        </div>
        <div className="panel p-4">
          <SectionTitle title="Intake channels" desc="Every request converges into a single triage queue." />
          <div className="grid grid-cols-3 gap-3 text-center text-xs">
            {(["APP", "SMS", "IVR"] as const).map((c) => (
              <div key={c} className="rounded-md border border-border p-3">
                <p className="text-lg font-semibold">{sosList.filter((s) => s.channel === c).length}</p>
      ...
      <div className="panel overflow-hidden">
        <div className="p-4">
          <SectionTitle title="Distress request register" desc="Click a row to inspect the priority breakdown." />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-sm">
            <thead className="bg-muted/60 text-[11px] uppercase tracking-wide text-muted-foreground">
              <tr>
                {["SOS ID", "Channel", "Location", "People", "Vulnerable", "Livestock", "Depth", "Priority", "Status"].map(
                  (h) => (
                ...
                .map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-xs">
                      <div className="font-medium text-foreground">{s.place}</div>
                      <div className="text-muted-foreground">
                        {s.district}, {s.state} · {s.lat.toFixed(4)}, {s.lng.toFixed(4)}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-2 tabular-nums">{s.floodDepthM} m</td>
      ...
      {selected && (
        <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <div className="panel p-4">
            ...
            {selected.raw && (
              <p className="mt-3 rounded-md bg-muted px-3 py-2 font-mono text-xs">{selected.raw}</p>
            )}
          </div>
          <div className="panel p-4">
            <SectionTitle title="Location context" desc="Flood layer, nearby resources and camps." />
            <MapPanel
              height={380}
              showControls={false}
